...
import { useGetSearchCourseQuery } from "@/featrues/api/courseApi";
...
const SearchPage = () => {
  ...
  const handleFilterChange = (categories, price) => {
    setSeletedCategories(categories);
    setSortByPrice(price);
  };
  return (
    <div className=" max-w-7xl mx-auto mt-4 p-4 md:p-8">
      <div className="my-6">
        <h1 className="font-bold text-xl md:text-2xl">Result for "{query}"</h1>
        <p>
          Showing Results for{" "}
          <span className=" text-blue-800 font-bold italic hover:underline">
            {query}
          </span>
        </p>
      </div>
        ...
        <div className=" flex-1">
          {isLoading ? (
            Array.from({ length: 3 }).map((_, idx) => (
              <CourseSkeletonForSearch key={idx} />
            ))
          ) : isEmpty ? (
            <CourseNotFound />
          ) : (
